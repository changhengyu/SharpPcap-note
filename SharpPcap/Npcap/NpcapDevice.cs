...
namespace SharpPcap.Npcap
{
    ...
    public class NpcapDevice : LibPcapLiveDevice
    {
        ...
        /// 只要有新的pcap统计信息可用于此Pcap设备，就会触发该事件。对于网络捕获的数据包，仅在“ PcapMode.Statistics”模式下工作时才调用此事件。
        /// </summary>
        ...
        ///通过后台线程启动捕获过程，将为每个捕获的数据包调用OnPacketArrival（）
        ...
        ///npcap设备可以捕获数据包或统计信息更新，因此，只有同时定义了数据包处理程序和统计信息处理程序，才会引发异常
        /// </summary>
        ...
        /// <summary>
        /// 将“PcapSendQueue”中的所有数据包发送到此Pcap设备
